...
for file_index,file in enumerate(fileList):
  ...
  for model_noise_index,model_noise in enumerate(noise_levels_model):
    ...
    if model_noise_index == 0:   # lowest noise
        neuron_idx = np.where( noise_levels_all[file_index] < model_noise + 0.5 )[0]
    elif model_noise_index == len(noise_levels_model)-1:   # highest noise
        neuron_idx = np.where( noise_levels_all[file_index] > model_noise - 0.5 )[0]
    else:
        neuron_idx = np.where( (noise_levels_all[file_index] > model_noise - 0.5) & (noise_levels_all[file_index] < model_noise + 0.5) )[0]
